...
import PopMenuHeaders from "../../gs12/components/PopMenuHeaders";
import PopMenuKode from "../../gs12/components/PopMenuKode";
import PopMenuSection from "../../gs12/components/PopMenuSection";
import PopMenuRow from "../../gs12/components/PopMenuRow";
import { forwardRef } from "react";
import PopMenuCell11 from "./PopMenuCell11";
...
export default function TabelPreviewAdmin11(props) {
  ...
  const total1 = sumBy(data, (r) => r.value);
      ...
      <table className="min-w-full table-fixed mb-4">
        ...
        <tbody>
                        ...
                        {kode.map((item, index) => {
                          ...
                          return (
                            ...
                            >
                              {f ? (
                                <div
                                  className={`py-2.5 mx-auto ${
                                    f.type === 1 && "bg-amber-200"
                                  }`}
                                >
                                  <TextField
                                  ...
                                  <div className="absolute inset-y-0 right-0 flex items-center opacity-0 group-hover:opacity-100">
                                    <PopMenuCell11
                                      statType={f.type}
                                      unsetSoal={() => setTypeCell(f.uuid, 0)}
                                      setSoal={() => setTypeCell(f.uuid, 1)}
          ...
          <tr className="bg-slate-50">
            ...
            <td className="text-center border table-cell bg-amber-200">
              <Tooltip title="Jawaban benar pada Worksheet mahasiswa" arrow>
                <div>
              ...
              <table className="w-full border-collapse">
                <tbody>
                  <tr>
                    {kode.map((item, index) => {
                      const val = sumBy(
                        data.filter((x) => x.idc === item.uuid),
                        (r) => r.value
                      );
                      const stat = find(data, {
                        type: 1,
                        idc: item.uuid,
                      });
                      ...
                      return (
                        ...
                        >
                          <Tooltip
                            title={
                              stat
                                ? "Jawaban benar pada Worksheet mahasiswa"
                                : ""
                            }
                            arrow
                          >
