...
import PopMenuHeaders from "../../componentglobal/PopMenuHeaders";
import PopMenuKode from "../../componentglobal/PopMenuKode";
import PopMenuSection from "../../componentglobal/PopMenuSection";
import PopMenuRow from "../../componentglobal/PopMenuRow";
import PopMenuCell from "../../componentglobal/PopMenuCell";
import PopMenuJumlah from "../../componentglobal/PopMenuJumlah";
import PopMenuRowStarGs13 from "./PopMenuRowStarGs13";
...
export default function TabelManagerAdmin(props) {
  ...
  const kpembantu3 = props.kpembantu.filter((x) => x.type === 3);
  ...
  const addKpembantu1 = (p, type) => {
    ...
    props.setKpembantu([...h, ...kpembantu2, ...kpembantu3]);
  };
  const addKpembantu2 = (p, type) => {
    ...
    props.setKpembantu([...kpembantu1, ...h, ...kpembantu3]);
  };
  const addKpembantu3 = (p, type) => {
    const h = [...kpembantu3];
    h.splice(p + 1, 0, {
      uuid: uuidv4(),
      alias: "",
    ...
    props.setKpembantu([...kpembantu1, ...kpembantu2, ...h]);
  };
  const removeKpembantu1 = (uid) => {
    const temp = remove(kpembantu1, (x) => x.uuid !== uid);
    props.setKpembantu([...temp, ...kpembantu2, ...kpembantu3]);
  ...
  const removeKpembantu2 = (uid) => {
    ...
    props.setKpembantu([...kpembantu1, ...temp, ...kpembantu3]);
    const temp1 = remove(data, (x) => x.idr !== uid);
    props.setData(temp1);
  };
  const removeKpembantu3 = (uid) => {
    const temp = remove(kpembantu3, (x) => x.uuid !== uid);
    props.setKpembantu([...kpembantu1, ...kpembantu2, ...temp]);
    const temp1 = remove(data, (x) => x.idr !== uid);
    props.setData(temp1);
  };
  const activeRow = (uuid) => {
    const tmp = kpembantu3.map((u, i) => ({
      ...u,
      status: false,
    }));
    ...
    props.setKpembantu([...kpembantu1, ...kpembantu2, ...tmp2]);
  };
  //#endregion ROW
  //#region CELL
  ...
  const drowpembantu = kpembantu3.filter(
        ...
        <tbody>
          ...
          <tr className="bg-slate-50">
            <td className="py-2 text-center border relative">
              Jumlah
              <div className="absolute inset-y-0 right-0 flex items-center">
                <PopMenuJumlah addRow={() => addKpembantu2(-1, 3)} />
              </div>
            </td>
              ...
              <table className="w-full border-collapse">
                <tbody>
                  <tr>
                    {kode.map((item, index) => {
                      const val = sumBy(
                        data.filter((x) => x.idc === item.uuid && x.type !== 3),
                        (r) => r.value
                      );
              ...
              >
                <td className="p-0 max-w-10v border group">
                  <div className="py-1 relative">
                    ...
                    <div className="absolute inset-y-0 right-0 flex items-center opacity-30 group-hover:opacity-100">
                      <PopMenuRowStarGs13
                        addRow={() => addKpembantu3(index, 3)}
                        removeRow={() => removeKpembantu3(trl.uuid)}
                        setSelect={() => {
                          activeRow(trl.uuid);
                        }}
                      />
                            ...
                            >
                              ...
                              dataInRowActiv[i].status ? (
                                ...
                                <div className="py-2.5 flex items-center opacity-5 group-hover:opacity-100">
                                  <Tooltip
                                    title="Pilih sebagai Acuan Soal"
                                    placement="top"
                                    arrow
                                  >
                                    <IconButton
            ...
            <td className="p-0">
              <table className="w-full border-collapse">
                <tbody>
                  <tr>
                    {kode.map((item, index) => {
                      const val2 = dataInRowActiv[index].value
                        ? dataInRowActiv[index].value
                        : 0;
                      const uidbl = drowpembantu[0] ? drowpembantu[0].uuid : "";
                      const val =
                        sumBy(
                          data.filter(
                            (x) => x.idc === item.uuid && x.idr !== uidbl
                          ),
                          (r) => r.value
                        ) + val2;
