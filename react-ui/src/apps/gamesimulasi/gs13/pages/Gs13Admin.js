...
import AdminBuktiMemorialGs13 from "../components/AdminBuktiMemorialGs13";
...
export default function Gs13Adminv2(props) {
        ...
        .then((res) => {
          ...
          if (!res.data.config.narasisoal) {
            const x = {
              ...res.data.config,
              narasisoal:
                '<p style="text-align:center;"><span style="font-size: 18px;"><strong>Game Simulasi 13 - Rekapitulasi dan Alokasi Biaya</strong></span></p>\n<p>Cara pengerjaan:<br>1) Isi Bukti Memorial, dari baris Kode Rekening pembantu <strong>71</strong><br>2) Isi Rekapitulasi jurnal</p>\n',
            };
            setDataConfig(x);
          } else {
            setDataConfig(res.data.config);
  ...
  const saveToDbGs13 = () => {
    ...
    const push = axios.post(
      `${API.HOST}/api/v2/manufakturgs13/data/update`,
      {
        idc: id,
        dataConf: dataConfig,
        headers: headers,
  ...
  return (
    <div className="w-full min-h-20v relative">
      ...
      <Button
        ...
        onClick={() => {
          if (dataAlokasi.length === 0) {
            toast.error("Data Kunci Jawaban masih Kosong isi terlebih dahulu");
          } else {
            saveToDbGs13();
          }
        }}
