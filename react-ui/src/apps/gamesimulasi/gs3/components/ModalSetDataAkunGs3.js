...
export default function ModalSetDataAkunGs3(props) {
  const classes = useStyles();
  const data = props.data;
  console.log(data);
  const [noAkun] = useState(data ? data.code : "");
  ...
  const deleted = () => {
    ...
    const pushToDelete = axios.post(
      `${API.HOST}/api/v2/manufakturgs3/databank/hapus`,
      {
        id: data.idbank,
      },
      {
  ...
  const updateData = () => {
    ...
    const pushToUpdate = axios.post(
      `${API.HOST}/api/v2/manufakturgs3/databank/update`,
      {
        id: data.idbank,
        // noakun: noAkun,
        akun: nama,
