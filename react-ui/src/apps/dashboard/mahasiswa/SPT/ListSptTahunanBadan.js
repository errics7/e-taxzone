...
const SptCreationWizard = () => {
    const [currentView, setCurrentView] = useState('list'); // 'list', 'create-step1', 'create-form', 'submitted', 'rejected', 'cancelled', 'pending-payment'
    const [selectedTaxType, setSelectedTaxType] = useState('');
    const [selectedPeriod, setSelectedPeriod] = useState('');
    const [selectedModel, setSelectedModel] = useState('');
    const [selectedPeriodType, setSelectedPeriodType] = useState('SPT Tahunan'); // 'SPT Tahunan' | 'SPT Sebagian'
    ...
    // File ini DEDICATED untuk SPT Badan (route: /home/spt-tahunan-badan-list)
    // isBadan selalu true — tidak bergantung currentSptType Redux
    // currentSptType tetap dibaca hanya untuk label di tabel row (baris 1419)
    const currentSptType = useSelector(selectCurrentSptType);
    const isBadan = true; // hardcoded: file ini khusus badan
    ...
    const createSpt = async () => {
        try {
            ...
            if (selectedModel === 'AMENDMENT') {
                ...
                setSelectedPeriod('');
                setSelectedModel('');
                setSelectedPeriodType('SPT Tahunan');
                setCurrentView('list');
            ...
            const payload = isBadan
                ? {
                    ...
                    tax_return_type: uiTaxType === 'dollar' ? 'Dollar' : 'Rupiah',
                    tax_period_type: selectedPeriodType === 'SPT Sebagian' ? 'Fractional' : 'Yearly'
                }
                : {
                    tax_year: parseInt(selectedPeriod),
                    tax_period: `${selectedPeriod} January - December`,
    ...
    const deleteSpt = async (sptId) => {
        try {
            setLoading(true);
            // FIX: endpoint delete sesuai portal aktif (isBadan selalu true di file ini)
    ...
    const submitSpt = async (sptId) => {
        try {
            setLoading(true);
            // FIX: endpoint submit sesuai portal aktif (isBadan selalu true di file ini)
    ...
    // canSaveForm() digunakan oleh CreateTaxReturnForm
                        ...
                        <div className="flex justify-center">
                            <div className="w-80">
                                <div className="border-2 border-yellow-400 rounded-lg p-6 bg-yellow-50">
                                    <h3 className="font-semibold text-gray-800 mb-4 text-lg">PPh Badan</h3>
                                    <label className="flex items-center cursor-pointer">
                                        <input
                                            type="radio"
                                            name="taxType"
                                            value="Business Activities"
                                            checked={selectedTaxType === 'Business Activities'}
    ...
    // ─── Coretax single form: state + helpers + CreateTaxReturnForm ─────────────
    const [uiTaxType, setUiTaxType] = useState('rupiah'); // 'rupiah' | 'dollar' — UI only

    const canSaveForm = () =>
        uiTaxType !== '' && selectedPeriod !== '' && selectedModel !== '' && selectedPeriodType !== '';

    const getReturnTypeLabel = () =>
        uiTaxType === 'dollar' ? 'SPT PPh Badan Dollar' : 'SPT PPh Badan Rupiah';

    const CreateTaxReturnForm = () => (
        <div className="min-h-screen bg-gray-50 flex">
            <Sidebar />
            <div className="flex-1 p-8">
                <div className="max-w-6xl mx-auto">
                ...
                <div className="max-w-3xl mx-auto">
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200">
                        ...
                        <div className="px-8 py-6 border-b border-gray-100">
                            <h1 className="text-2xl font-semibold text-gray-800">Create Tax Return</h1>
                        </div>
                        {/* Form body */}
                        <div className="px-8 py-6 space-y-5">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <select
                                    value={uiTaxType}
                                    onChange={(e) => setUiTaxType(e.target.value)}
                                    className="flex-1 px-3 py-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm bg-white"
                                >
                                    <option value="rupiah">Pajak Penghasilan Badan dalam Rupiah</option>
                                    <option value="dollar">Pajak Penghasilan Badan dalam Dollar</option>
                                </select>
                            </div>
                            {/* 2. Tax Return Type — auto dari Tax Type, read-only */}
                            <div className="flex items-center gap-4">
                                <label className="w-56 text-sm text-gray-700 flex-shrink-0">
                                    Tax Return Type <span className="text-red-500">*</span>
                                </label>
                                <div className="flex-1 px-3 py-2 border border-gray-200 rounded bg-gray-50 text-sm text-gray-700">
                                    {getReturnTypeLabel()}
                                </div>
                            </div>
                            {/* 3. Tax Return Period Type — dropdown */}
                            <div className="flex items-center gap-4">
                                <label className="w-56 text-sm text-gray-700 flex-shrink-0">
                                    Tax Return Period Type <span className="text-red-500">*</span>
                                </label>
                                <select
                                ...
                                >
                                    ...
                                    <option value="SPT Sebagian" disabled>SPT Sebagian</option>
                                </select>
                            </div>
                            {/* 4. Tax Period → selectedPeriod, dikonsumsi createSpt() */}
                            <div className="flex items-center gap-4">
                                <label className="w-56 text-sm text-gray-700 flex-shrink-0">
                            ...
                            {/* 5. Tax Return Model → selectedModel, dikonsumsi createSpt() */}
                            <div className="flex items-center gap-4">
                                <label className="w-56 text-sm text-gray-700 flex-shrink-0">
                                    Tax Return Model <span className="text-red-500">*</span>
                                </label>
                                <select
                                    value={selectedModel}
                        ...
                        <div className="px-8 py-5 border-t border-gray-100 flex items-center gap-3">
                            <button
                                ...
                                className="bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white px-6 py-2 rounded font-medium text-sm transition-colors"
                            >
                            ...
                            <button
                                onClick={() => {
                                    ...
                                    setUiTaxType('rupiah');
                                    setSelectedPeriodType('SPT Tahunan');
                                    setCurrentView('create-step1');
                                }}
                                disabled={loading}
                                className="bg-gray-500 hover:bg-gray-600 disabled:opacity-50 text-white px-6 py-2 rounded font-medium text-sm transition-colors"
                            >
                                Kembali
                            </button>
                        </div>
                    ...
                    <div className="p-6">
                        ...
                        <div className="flex items-center justify-between mb-6">
                            <div className="flex gap-3">
                                {showCreateButton && (
                                    <button
                                        onClick={() => setCurrentView('create-step1')}
    ...
    const renderCurrentView = () => {
        switch (currentView) {
            case 'create-step1': return <Step1View />;
            case 'create-form': return <CreateTaxReturnForm />;
