...
import dummyGS6 from "./Data";
import TableWorksheetMhs6 from "../components/TableWorksheetMhs6";
import TableWorksheetRekapMhs6 from "../components/TableWorksheetRekapMhs6";
import NotaListTableMhs from "../components/NotaListTableMhs";
import ShimmerWorksheetMhs5 from "../../gs5/components/ShimmerWorksheetMhs5";
import ShimmerSoalMhs5 from "../../gs5/components/ShimmerSoalMhs5";
...
export default function Gs5Mhs() {
  ...
  useEffect(() => {
    ...
    const fetchData = () => {
      axios(`${API.HOST}/api/v2/gamesimulasi6/${id}/config`, {
        ...
        .catch((error) => {
          prepareData2(dummyGS6.dataakun);
          prepareData(dummyGS6);
          setLoad(false);
  ...
  const prepareData = (inn) => {
    ...
    const jwb1 = inn.datanota.map((item, index) => ({
      ...
      soal_persediaan: item.type === "kontan" ? item.subtotal : 0,
      soal_ppn: item.type === "kontan" ? item.ppn : 0,
      soal_kas: item.type === "kontan" ? item.total : item.nilaia,
      soal_bebangaji: item.type === "kontan" ? 0 : item.nilaia,
  ...
  const check1 = () => {
    ...
    jawab1.forEach((element, index) => {
      ...
      if (element.type === "kontan") {
        ...
        // #2 Piutang Dagang
        if (Number(element.soal_bebangaji) === Number(element.jwb_bebangaji)) {
          result.push(true);
        } else {
          jawab1[index].err_bebangaji = true;
          result.push(false);
        }
      }
      //end
  ...
  return (
    <div className="w-full min-h-20v relative">
      <Helmet>
        <title>Game Simulasi 6 Perdagangan</title>
      </Helmet>
      <>
        <Button
          variant="contained"
        ...
        <Grid container spacing={2} direction="row" alignItems="stretch">
          ...
          <Grid item xs={12} md={12} lg={12} className="border">
            <div className="bg-white">
              ...
              {config ? (
                <>
                  <NotaListTableMhs dataConfig={config} />
                </>
              ) : (
                <ShimmerSoalMhs5 />
              )}
