...
import ShimmerWorksheetMhs15 from "../component/ShimmerWorksheetMhs15";
import TableSoalMhs15 from "../component/TableSoalMhs15";
import TabelWorksheetMhs15 from "../component/TabelWorksheetMhs15";
...
export default function Gs15Mhs() {
  ...
  const prepareData = (inn) => {
    //#1 Data Nilai
    const tmpjwb = [];
    inn.datanilai.forEach((el) => {
      if (el.key) {
        const cari = find(inn.dataakun, { uid: el.idr });
        tmpjwb.push({
          ...el,
          used: false,
          sec: "akun",

          key_noakun: cari.noakun,
          key_alias: cari.alias,
          key_value: el.value,
  ...
  const checkjwb = () => {
    ...
    jwbdata.forEach((el, i) => {
      if (el.sec === "total") {
        ...
        //1 cari noakun sama
        const idx = findIndex(jwbdata, { key_noakun: el.jwb_noakun.trim() });

        if (idx >= 0 && jwbdata[idx].used === false) {
          //langsung used truee
          jwbdata[idx].used = true;
          //2 cek nokakun posisi benar salah deb/kre
          if (jwbdata[idx].type === el.type) {
            //3 cek alias
            if (
              jwbdata[idx].key_alias.trim().toLowerCase() ===
              el.jwb_alias.trim().toLowerCase()
            ) {
              result.push(true);
            } else {
            ...
            //4 cek Value / jumlah
            if (Number(jwbdata[idx].key_value) === Number(el.jwb_value)) {
              result.push(true);
            } else {
              jwbdata[i].err_value = true;
              result.push(false);
  ...
  return (
    <div className="w-full min-h-20v relative">
      <Helmet>
        <title>Game Simulasi 15 Perdagangan</title>
      </Helmet>
      <>
        <Button
          variant="contained"
        ...
        <Grid item xs={12} md={12} lg={12}>
          {config && jwbdata ? (
            <>
              ...
              <TableSoalMhs15 dataConfig={config} />
