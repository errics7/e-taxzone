...
import PopMenuCell15 from "./PopMenuCell15";
import PopMenuRowWorksheet15 from "./PopMenuRowWorksheet15";
...
export default function TableSoalAdmin15(props) {
  ...
  const addData = (idc, idr, type) => {
    const list = [...dataConfig.datanilai];
    const key = dataConfig.keytabel === idc;
        ...
        <table className="border-collapse min-w-full table-fixed">
                          ...
                          <td className="min-w-15v max-w-15v p-1 text-center text-slate-800 border border-b relative group">
                            {fd && fd.type === "debet" ? (
                              <div className={`${fd.key && " bg-amber-200"}`}>
                                ...
                                />
                                <div className="absolute inset-y-0 right-0 flex items-center opacity-0 cursor-pointer z-50 group-hover:opacity-100">
                                  <PopMenuCell15
          ...
          <tfoot>
            ...
            <tr className="hover:bg-slate-100 bg-slate-50 font-semibold">
              <td
                colSpan="2"
                className="p-1.5 uppercase text-center text-slate-800 border border-b"
              >
                Total
              </td>
              {dataConfig.dataheader.map((dat, ii) => {
                const ky = dataConfig.keytabel === dat.uid;
                return (
                  <Fragment key={ii}>
                    <td className="min-w-15v max-w-15v p-0.5 text-center text-slate-800 border border-b">
                      <Tooltip
            ...
            <tr className="hover:bg-slate-100 bg-slate-50 font-semibold">
              <td
                colSpan="2"
                className="p-2 uppercase text-center text-slate-800 border border-b"
              >
                LABA
              </td>
              {dataConfig.dataheader.map((dat, ii) => {
                ...
                const iskey = dat.uid === dataConfig.keytabel;
                return (
                  <Fragment key={ii}>
                    <td className="min-w-15v max-w-15v p-0.5 text-center text-slate-800 border border-b">
                      {ky && diff && diff[1] === "debet" && (
                        <Tooltip
                          title={
                            iskey
                              ? "Input Nilai jawaban benar untuk Mahasiswa"
                              : ""
                          }
                        >
                          <div className={`p-1.5 ${iskey && " bg-amber-200"}`}>
                            {numberFormat(diff[0])}
                          </div>
                        </Tooltip>
                      )}
