...
function TableKas(props) {
  const classes = useStyles();
  const { dataConfig, setdataConfig } = props;
//   const filterDataJurnal = dataConfig && dataConfig.datajurnal.filter(item => item.type === "KAS KELUAR")
  const Ppersediaan = dataConfig && filter(dataConfig.datajurnal, { type: "jurnal kas keluar" }).map((item) => parseInt(item.persediaan));
  const Pppnmasukan = dataConfig && filter(dataConfig.datajurnal, { type: "jurnal kas keluar" }).map((item) => parseInt(item.persediaan/10));
  const Pkas = dataConfig && filter(dataConfig.datajurnal, { type: "jurnal kas keluar" }).map((item) => parseInt(item.persediaan) + parseInt(item.persediaan/10));
  ...
  const handleRemoveItemSoal = (idx) => {
    // assigning the list to temp variableX
  ...
  const addRowBarangKas = () => {
    ...
    const dict = ["persediaan", "ppnmasukan", "kas"];
      ...
      <>
        <div className="mt-3 overflow-x-auto border-collapse border">
          <table className="border-collapse min-w-full table-fixed">
            ...
            <tbody>
              {dataConfig &&
                filter(dataConfig.datajurnal, { type: "jurnal kas keluar" }).map((item, index) => (
                  <tr
                    key={index}
                  ...
                  >
                    <td className="lg:w-auto  text-slate-800 text-center border border-b">
                      <div className="relative">
                         {/* {numberFormat(item.subtotal)} {item.hutangdagang} */}
          ...
          <Button
            ...
            onClick={() => {
              const temp = [...dataConfig.datajurnal];
              temp.filter(x => x.type === 'jurnal kas keluar').forEach(x => temp.splice(temp.indexOf(x), 1));
              const deleteAkun = [...dataConfig.dataakun].map(item => {
                const tempakun = [...item.idakun]
                if(remove(tempakun)) {
                  return {
