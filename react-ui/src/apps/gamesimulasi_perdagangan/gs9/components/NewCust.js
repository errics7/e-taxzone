  ...
  const customerBaru = () => {
    ...
    const newlist = [
      ...
      },
      {
        ...
        key: "hutangdagang",
        posisi: title === "jurnal kas keluar" ? "debit" : "kredit",
        jumlah: 0, //only type intro
      },
    ];
      ...
      <Fade in={props.openn}>
        <div className="relative bg-white outline-none rounded-lg w-3/4 min-h-30v  flex flex-col items-center">
          <div className="relative inset-y-0 top-0 flex flex-row w-full items-center border-b justify-center">
            <h2 className="flex-none text-center capitalize font-medium text-2xl py-5">
              Data Baru {title}
            </h2>
          ...
          <div className="p-5 flex flex-col w-full mx-auto">
            ...
            <p className=" text-center w-36 mx-auto my-5">
              Atau Pilih dari data Pemasok yang sudah Ada
            </p>
              ...
              {objJurnal.map((el, i) => {
                ...
                return (
                  <div
                    key={i}
                    onClick={() => {
                      customerLama(el.head, da.namapemasok);
                    }}
