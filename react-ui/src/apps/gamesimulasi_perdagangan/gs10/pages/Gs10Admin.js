...
import TableTransaksiAdmin10 from "../component/TableTransaksiAdmin10";
import ShimmerAdmin10 from "../component/ShimmerAdmin10";
import KartuPersediaanAdmin10 from "../component/KartuPersediaanAdmin10";
...
export default function Gs12Admin(props) {
  ...
  const defaultDataCheck = (inn) => {
    ...
    if (conf.narasisoal === null || conf.narasisoal === "") {
      conf = {
        ...conf,
        narasisoal: `<p><span style="color: rgba(0,0,0,0.87);background-color: rgb(255,255,255);font-size: 14px;font-family: Arial;"><strong>GAME SIMULASI 10 - </strong></span><span style="color: rgb(0,0,0);background-color: rgb(255,255,255);font-size: 13px;font-family: Arial;"><strong>MENCATAT MUTASI PERSEDIAAN PADA KARTU PERSEDIAAN</strong></span><span style="font-family: Arial;"> </span></p>`,
      };
    }
    if (conf.introsoal === null || conf.introsoal === "") {
      conf = {
        ...conf,
        introsoal: `Perusahaan menerapkan salah satu kebijakan akuntansi yaitu: Persediaan Barang Dagangan dicatat dengan Metode Perpetual dan metode penilaiannya menggunakan menggunakan metode FIFO.`,
      };
    }
    if (conf.databarang && conf.databarang.length === 0) {
      conf = {
        ...conf,
        selectedbrg: variatif[0],
        databarang: [
          {
            uid: uuidv4(),
            gen: variatif[0],
  ...
  const saveToDb = () => {
    ...
    const push = axios.post(
      `${API.HOST}/api/v2/gamesimulasi10/${id}/update`,
      {
        config: config,
      },
      {
  ...
  return (
    <div className="w-full min-h-20v relative">
      <Helmet>
        <title>Perdagangan 10 | Admin</title>
      </Helmet>
      <div className="flex flex-col items-start relative">
        ...
        <div className="relative w-full mb-5">
          <div className="text-2xl text-center w-full absolute">
            Konfigurasi Game Simulasi Perdagangan 10
          </div>
        </div>
      </div>
            ...
            {config ? (
              <>
                <div className="relative pb-3 border-b">
                  <div className="absolute opacity-50 bg-blue-200 italic font-semibold -mt-5 p-1 pr-2">
                    Admin Soal :
                  </div>
                  <TableTransaksiAdmin10
                    dataConfig={config}
                    setdataConfig={(dat) => setConfig(dat)}
                  />
                </div>
