...
import TableWorksheetMhs4 from "../components/TableWorksheetMhs4";
import TableWorksheetRekapMhs4 from "../components/TableWorksheetRekapMhs4";
import TableHppMhs from "../components/TableHppMhs";
import InvoiceListTableMhs from "../components/InvoiceListTableMhs";
import ShimmerSoalMhs4 from "../components/ShimmerSoalMhs4";
import ShimmerWorksheetMhs4 from "../components/ShimmerWorksheetMhs4";
...
export default function Gs4Mhs() {
  ...
  const prepareData = (inn) => {
    // console.log(inn);
    const jwb1 = inn.datainvoice.map((item, index) => ({
      ...item,
      soal_piutangdagang: item.jumlah,
      soal_hpp: item.hpp,
      soal_penjualan: item.subtotal,
      soal_ppnkeluaran: item.ppn,
      soal_persediaan: item.persediaan,
      jwb_piutangdagang: "",
      jwb_hpp: "",
      jwb_penjualan: "",
      jwb_ppnkeluaran: "",
  ...
  const check1 = () => {
    ...
    jawab1.forEach((element, index) => {
      // #1 Hutang Dagang
      if (Number(element.jumlah) === Number(element.jwb_piutangdagang)) {
        result.push(true);
      } else {
      ...
      // #2 HPP
      if (Number(element.hpp) === Number(element.jwb_hpp)) {
        result.push(true);
      } else {
      ...
      // #3 Penjualan
      if (Number(element.subtotal) === Number(element.jwb_penjualan)) {
        result.push(true);
      } else {
      ...
      // #4 PPN Keluaran
      if (Number(element.ppn) === Number(element.jwb_ppnkeluaran)) {
        result.push(true);
      } else {
        jawab1[index].err_ppnkeluaran = true;
        result.push(false);
      }
      // #5 Persediaan
      if (Number(element.persediaan) === Number(element.jwb_persediaan)) {
        result.push(true);
      } else {
  ...
  return (
    <div className="w-full min-h-20v relative">
      <Helmet>
        <title>Game Simulasi 4 Perdagangan</title>
      </Helmet>
      <>
        <Button
          variant="contained"
        ...
        <Grid container spacing={2} direction="row" alignItems="stretch">
          ...
          <Grid item xs={12} md={12} lg={12}>
            <div className="bg-white border">
              ...
              {config ? (
                <>
                  <TableHppMhs dataConfig={config} />
                  <InvoiceListTableMhs dataConfig={config} />
                </>
              ) : (
                <ShimmerSoalMhs4 />
              )}
