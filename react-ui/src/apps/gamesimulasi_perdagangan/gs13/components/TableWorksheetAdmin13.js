...
import { filter, find, findIndex, remove, sumBy } from "lodash";
import { forwardRef, Fragment } from "react"; 
import EditIcon from "@mui/icons-material/Edit";
import Addicon from "@mui/icons-material/AddCircle";
import PopMenuCell13 from "./PopMenuCell13";
import PopMenuRowWorksheet13 from "./PopMenuRowWorksheet13";
...
export default function TableWorksheetAdmin13(props) {
  ...
  const addData = (idc, idr, type, key) => {
    const list = [...dataConfig.datanilai];
    list.splice(0, 0, {
      uid: uuidv4(),
      idc: idc,
  ...
  const removeDataAkun = (uid) => {
    ...
    const listsoal = [...dataConfig.datasoal];
    remove(listnilai, (x) => x.idr === uid);
    remove(list, (x) => x.uid === uid);
    const newsoal = listsoal.map((el) => {
      const ttmp = [...el.list];
      remove(ttmp, (x) => x === uid);
      return {
        ...el,
              ...
              {dataConfig.dataheader.map((dat, ii) => (
                <Fragment key={ii}>
                  <td className="min-w-15v max-w-15v p-0.5 text-center text-slate-800 border border-b">
                    <div
                      className={`p-1.5 ${
                        dat.uid === dataConfig.selectedwork && " bg-amber-200"
                      }`}
                    >
                      {numberFormat(
                        sumBy(
