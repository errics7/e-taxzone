...
const { unproject } = (function unprojectFunction() {
  ...
  return {
    unproject(THREE, vector, camera) {
      ...
      // console.log(matrix.getInverse(threeCamera.projectionMatrix));
      // vector.applyProjection(matrix.inverse(threeCamera.projectionMatrix));
...
const { directionToWorldCoords } = (function directionToWorldCoordsFunction() {
  ...
  return {
    ...
    directionToWorldCoords(
      ...
      // A line from the camera position toward (and through) the plane
      //add
      var direction_c = new THREE.Vector3();
      camera.getWorldDirection(direction_c);
      // console.log(direction_c);
...
const { selectItem } = (function selectItemFunction() {
  ...
  return {
    selectItem(THREE, selector, camera, clientX, clientY) {
      ...
      //ADD camera.components.camera.camera
      ...
      //ADD same
      var worldPos_object = new THREE.Vector3();
      worldPos_object.setFromMatrixPosition(object.matrixWorld);

      //   const offset = point.sub(object.getWorldPosition());
      const offset = point.sub(worldPos_object);
...
function dragItem(THREE, element, offset, camera, depth, mouseInfo) {
  ...
  var camQ_new = new THREE.Quaternion();
  var ellQ_new = new THREE.Quaternion();
  camQ_new.setFromRotationMatrix(camera.object3D.matrixWorld);
  ellQ_new.setFromRotationMatrix(element.object3D.matrixWorld);

  // Setting up for rotation calculations
  const startCameraRotationInverse = camQ_new.invert();
  //   const startCameraRotationInverse = threeCamera.getWorldQuaternion().inverse();
  const startElementRotation = ellQ_new;
  ...
  function onMouseMove({ clientX, clientY }) {
    ...
    //add
    var treeQ_new = new THREE.Quaternion();
    treeQ_new.setFromRotationMatrix(threeCamera.matrixWorld);
