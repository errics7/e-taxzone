...
const gs5_data = require("../../../models/gsak1/gs5_data.model");
...
exports.selected = async (req, res) => {
  ...
  // #1
  var dataConfig = await gs5_config
    .findOne({
      where: { id: tagid },
      raw: true,
    })
...
exports.updategsdata = async (req, res) => {
  ...
  // console.log(req.body.dataconf);
  // console.log(req.body.data);
  const data = req.body.data;
  const dataconf = req.body.dataconf;

  // removing old
  var proc1 = await gs5_data
    .destroy({
    ...
    .then(async () => {
      ...
      //  BAtch
      if (cdata.length > 0) {
        await gs5_data.bulkCreate(cdata).catch((error) => {
          res.status(500).json({
            success: false,
            message: error,
          });
    ...
    .update(
      {
        ...
        keterangan: dataconf.keterangan,
        kwt: dataconf.kwt,
        harga: dataconf.harga,
