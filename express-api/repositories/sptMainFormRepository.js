/**
 * SptMainFormRepository
 *
 * Persistence / Data Access Layer for spt_main_form.
...
const spt_main_form = require("../models/spt_main_form.model");

class SptMainFormRepository {
  /**
  ...
  async create(data, options = {}) {
    const { transaction } = options;
    return spt_main_form.create(data, { transaction });
  }
  ...
  async updateById(id, data, options = {}) {
    const { transaction } = options;
    const record = await spt_main_form.findOne({
      where: { id, deleted_at: null },
      transaction,
    });
...
module.exports = new SptMainFormRepository();
