...
/**
 * SptL13bSectionBRepository
 * Persistence layer for 'spt_l13b_section_b'.
...
const Model = require("../models/spt_l13b_section_b.model");

class SptL13bSectionBRepository {
  /**
   * findById(id, options = {})
  ...
  /**
   * findAllByL13bId(l13bId, options = {})
   * -> Model[]
   */
  async findAllByL13bId(l13bId, options = {}) {
    const { transaction } = options;

    return Model.findAll({
      where: { l13b_id: l13bId },
      transaction,
    });
  }

  /**
   * findByL13bAndCategoryCode(l13bId, categoryCode, options = {})
   * -> Model instance or null
   */
  async findByL13bAndCategoryCode(l13bId, categoryCode, options = {}) {
    const { transaction } = options;

    return Model.findOne({
      where: { l13b_id: l13bId, category_code: categoryCode },
      transaction,
    });
  }
}

module.exports = new SptL13bSectionBRepository();
